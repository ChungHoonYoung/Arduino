...
void returnAngleButton() {
  if (digitalRead(PIN_RETURN_BUTTON) == LOW) {
    String command = String(baseAngle) + "," + String(shoulderAngle) + "," + String(upperarmAngle) + "," + String(forearmAngle) + "d";
    Serial.println(command);
    while (!digitalRead(PIN_RETURN_BUTTON))
      delay(100); // 버튼이 계속 눌려있는 동안 대기
  }
}
