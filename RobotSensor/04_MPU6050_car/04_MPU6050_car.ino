...
void loop() {
  ...
  int iSpeed = 100;
  int leftWheelSpeed = iSpeed;
  int WHEEL_DIFF_VALUE = (g.gyro.z * 100) +10;
  int rightWheelSpeed = leftWheelSpeed + WHEEL_DIFF_VALUE;

  analogWrite(PIN_MA1A, leftWheelSpeed);
  analogWrite(PIN_MA1B, 0);
  analogWrite(PIN_MB1A, rightWheelSpeed);
  analogWrite(PIN_MB1B, 0);
